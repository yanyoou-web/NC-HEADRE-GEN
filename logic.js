...
const Logic = (() => {
  ...
  const kikaiMaster = {
    ...
    "NCL-015": { tumeNagasa: 50, tetsugetaTakasa: 5,  tetsugetaNo: "T-1", offset: 126.181,  kishu: "森精機CL-150" },
  ...
  const tetsugetaMaster = {
    ...
    "T-10": { takasa: 19, haba: 13.5, ana: "なし" },
  ...
  /* ----------------------------------------------------
     ...
     最終行は G10L2P0Z...(WORK-OFFSET)M1(NYUURYOKU) 形式になる。
  ---------------------------------------------------------- */
  ...
  /* ----------------------------------------------------
     7) 作成者名 マスター
     ここに名前を追加・削除すると、フォームのプルダウンに反映されます。
  ---------------------------------------------------------- */
   ...
   *   アテを高さ差が小さい順に並べておすすめとして返す。
   *   Φ=13 のアテは画面には表示するが、おすすめ候補からは除外する
   *   （細径すぎて選定対象外のため）。
   *   目安が計算できない場合は高さ順で全件を返す。
   *
   * 受け取る値：
  ...
  function generateOutputText(data) {
    ...
    const lines = [
      ...
      `G10L2P0Z${fmt(offsetValue)}(WORK-OFFSET)M1(NYUURYOKU)`,
    ];
